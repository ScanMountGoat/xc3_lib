...
### xc3_model
xc3_model provides an abstraction over xc3_lib that is easier to read, edit, and convert. xc3_model uses higher level abstractions like a model, texture, or animation that may encompass multiply binary format types in xc3_lib. Similar to xc3_lib, the public API in xc3_model almost exclusively uses plain data with public fields and no encapsulation to simplify testing and consuming code. xc3_model attempts to fully represent the underlying xc3_lib data, so testing is done via roundtrip conversions in game files.
