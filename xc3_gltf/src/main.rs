...
fn main() -> anyhow::Result<()> {
    ...
    match Path::new(&cli.input).extension().unwrap().to_str().unwrap() {
        "wimdo" => {
            ...
            if is_glb {
                export_model_glb(root, &name, output, false)
            } else {
                export_model_gltf(root, &name, output, false)
            }
        }
        "pcmdo" => {
            let root = load_model(&cli.input, database.as_ref())
        ...
        "camdo" => {
            ...
            if is_glb {
                export_model_glb(root, &name, output, true)
            } else {
                export_model_gltf(root, &name, output, true)
            }
        }
        "wismhd" => {
...
fn export_model_gltf(
    ...
    GltfFile::from_model(name, &[root], flip_images_uvs)
        .with_context(|| "failed to create glTF file")?
        .save(output)
        .with_context(|| format!("failed to save glTF file to {:?}", output))
}
...
fn export_model_glb(
    ...
    GlbFile::from_model(name, &[root], flip_images_uvs)
        .with_context(|| "failed to create glb file")?
        .save(output)
        .with_context(|| format!("failed to save glb file to {:?}", output))
}
